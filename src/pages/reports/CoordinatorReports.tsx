...
import { FolderOpen, Layers, CheckCircle, AlertTriangle, List, Clock } from "lucide-react";

/** Conteo genérico: recibe un arreglo y cuenta por una clave */
...
export default function CoordinatorReports() {
  ...
  async function loadData() {
    ...
    const [projRes, stagesRes, deadlinesRes] = await Promise.all([
      supabase.from("projects").select("id, global_status"),
  ...
  const officialLabels: Record<string, string> = {
    APROBADA: "Aprobada",
    APROBADA_CON_MODIFICACIONES: "Con Modificaciones",
    ...
    <div className="space-y-8">
      ...
      <section>
        <h2 className="text-lg font-semibold mb-3 flex items-center gap-2">
          <FolderOpen className="h-4 w-4" /> Proyectos por Estado
        </h2>
      ...
      <section>
        <h2 className="text-lg font-semibold mb-3 flex items-center gap-2">
          <Layers className="h-4 w-4" /> Proyectos por Etapa Actual
        </h2>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
      ...
      <section>
        <h2 className="text-lg font-semibold mb-3 flex items-center gap-2">
          <AlertTriangle className="h-4 w-4" /> Plazos
        </h2>
        <div className="grid grid-cols-3 gap-4">
          ...
          <Card className="border-warning/30">
            <CardContent className="py-4">
              ...
              <p className="text-xs text-muted-foreground mt-1">Por Vencer (≤3 días)</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="py-4">
